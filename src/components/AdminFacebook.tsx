...
export default function AdminFacebook({
  ...
  const [customApprovalRate, setCustomApprovalRate] = React.useState<number>(settings.facebookRatePerId !== undefined ? settings.facebookRatePerId : (settings.ratePerId || 45));
  ...
  React.useEffect(() => {
    const defaultFbRate = settings.facebookRatePerId !== undefined ? settings.facebookRatePerId : (settings.ratePerId || 45);
    setCustomApprovalRate(defaultFbRate);
  }, [settings.facebookRatePerId, settings.ratePerId]);
  ...
  // Export filtered submissions as Excel (CSV) based on selected status mode
  const handleExportFilteredCSV = () => {
    ...
    link.setAttribute("download", `facebook_${exportStatusMode}_ids${pwdTag}_${new Date().toLocaleDateString()}.csv`);
  ...
  return (
    <div className="space-y-6">
      ...
      <ApprovalRateModal
        ...
        defaultRate={customApprovalRate}
        categoryName="Facebook (পেস্টকৃত)"
        mode="approve"
      />
    </div>
  );
